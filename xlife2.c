...
int main(int argc, char** argv){
  ...
  if (world_rank == 0){
	...
	fprintf(stderr, "les variables sont %d,%d,%d,%d,%d,%d,%d,%d,%d\n", n,kp,k_col,k_row,prochain_expand_col, prochain_expand_row,nb_step,row,col);
      }
  }
  ...
  for( s=1;s<=nb_step;s++){
	...
	{
	  for (j = 0; j<k_col; j++)
	    fprintf(stderr, "avant envoi, proc %d, case (%d,%d) il y a %d\n", world_rank, i, j, mydata[i][j]);
	}
      fprintf(stderr, "nord = %d, sud = %d, est = %d, ouest = %d\n", nord, sud, est, ouest);
    }
    MPI_Barrier(MPI_COMM_WORLD);
	...
	{
	  for (j = 0; j<k_col; j++)
	    fprintf(stderr, "apres envoi, proc %d, case (%d,%d) il y a %d\n", world_rank, i, j, mydata[i][j]);
	}
    }
    //étape    
